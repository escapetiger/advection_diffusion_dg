function res = assemble_diffusion_pri_pos(par, ref, dim, hx)
%ASSEMBLE_DIFFUSION_PRI_POS Assemble diffusion matrix for a primal 
% variable using positive flux along a dimension.
%
%   This function computes the diffusion matrix for a primal variable 
%   using positive fluxes along the specified spatial dimension.
%
%   Syntax:
%      res = assemble_diffusion_pri_pos(par, ref, dim, hx)
%
%   Inputs:
...
if par.bc(dim) == 0 % periodic B.C.
    ...
    cols(a+i) = repmat(c, nc, 1) + kron((0:nc - 1)'*nl, ones(numel(c), 1));
    vals(a+i) = repmat(v, nc, 1);
elseif par.bc(dim) == 1 % Dirichlet B.C.
    m = multi_index(par.nx);
    m = m(m(:, dim) > 1, :);
    ni = size(m, 1);
    i = 1:numel(v) * ni;
